...
def render_student_evolution(quiz_id, student_name):
    ...
    for q_idx in range(num_questions):
        ...
        # # 2. Inject the "Quiz Started" marker (White Circle)
        # plot_data.append({
        #     "Time": reference_start_time,
        #     "LapseLabel": reference_start_time.strftime("%d/%m/%Y<br>%H:%M"),
        #     "Test Case": "Quiz Start",
        #     "Status": "Opened"
        # })
        #
        # # element = len(student_data.questions[0].test_results)
        # # print(element, type(element))
        # for t_idx in range(len(student_data.questions[0].test_results)):
        #     plot_data.append({
        #         "Time": reference_start_time,
        #         "LapseLabel": str(t_idx),
        #             # reference_start_time.strftime("%d/%m/%Y<br>%H:%M"),
